...
icon = pygame.image.load('img/bigslick.jpg')
...
class Play:
    ...
    def hit(self):
        ...
        if self.player_card == 4:
            self.player.calc_hand()
            self.player.display_cards()
            player_card_4 = pygame.image.load('img/' + self.player.card_img[4] + '.png').convert()
            gameDisplay.blit(player_card_4, (740, 450))
...
while running:
    for event in pygame.event.get():
        ...
        game_texts("Welcome to a game of Blackjack:", int(display_width/1.5), 70)
